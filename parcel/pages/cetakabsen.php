...
$sqk = mysql_query("select * from cbt_tahunakademik group by Xtahunakadmik");
$row=mysql_fetch_array($sqk,MYSQL_ASSOC);
...
$logsek = $ad['XLogo'];
   ...
   $this->Image('../../images/'.$logsek,1,1,2); // logo
   $this->SetTextColor(0,0,0); // warna tulisan
   $this->SetFont('arial','B','14'); // font yang digunakan
   // membuat cell dg panjang 19 dan align center 'C'
   $this->Cell(3,1,''); // cell dengan panjang 1
   $this->Cell(13,1,'DAFTAR HADIR PESERTA USBK ',0,0,'C');
   $this->Ln(0.5);
      $this->SetTextColor(0,0,0); // warna tulisan
   $this->SetFont('Arial','B','14'); // font yang digunakan
   // membuat cell dg panjang 19 dan align center 'C'
   $this->Cell(3,1,''); // cell dengan panjang 1
   $this->Cell(13,1,'UJIAN SEKOLAH BERBASIS KOMPUTER',0,0,'C');
      $this->Ln(0.5);
      $this->SetTextColor(0,0,0); // warna tulisan
   ...
   $this->Cell(3,1,''); // cell dengan panjang 1 
   $this->Cell(13,1,' TAHUN PELAJARAN ' .$tahun,0,0,'C');
   $this->Ln(1.5);
   ...
   $this->Ln();
   $this->Cell(3.8,0.7,'Hari/Tanggal','',0,'L');

   $this->SetFont('Arial','','10');
   $this->Cell(9,0.7,': '.$hari.','.$tgl.' '.$bln.' '.$thn.' ',0,'L');
   $this->SetFont('Arial','','10');
  ...
  function Footer(){
   $this->SetY(-2,5);
   $this->Cell(0,1,'SMK BINA MANDIRI hal   '. $this->PageNo(),0,0,'C');
  } 
 }
...
if(isset($_REQUEST['kelas'])){ 
 $q = mysql_query("SELECT * FROM cbt_siswa where XKodeSekolah = '$_REQUEST[server]' and XKodeKelas = '$_REQUEST[kelas]' and  XKodeJurusan = '$_REQUEST[jur]'  and  XSesi = '$_REQUEST[sesi]' and  XRuang = '$_REQUEST[ruang]'");
 } else {
