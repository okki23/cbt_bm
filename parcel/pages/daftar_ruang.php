...
$sql = mysql_query("delete from cbt_ruang where Urut = '$_REQUEST[urut]'");
}


if(isset($_REQUEST['simpan'])){
	$sql = mysql_query("update cbt_ruang set XRuang = '$_REQUEST[txt_ruang]' where Urut = '$_REQUEST[urut]'");
}

if(isset($_REQUEST['tambah'])){

$sqlcek = mysql_num_rows(mysql_query("select * from cbt_ruang where XRuang = '$_REQUEST[txt_ruang]'"));
	if($sqlcek>0){
	$message = "Kode Ruang SUDAH ADA";
	echo "<script type='text/javascript'>alert('$message');</script>";
	} else {
		if(!$_REQUEST['txt_ruang']==""){
		$sql = mysql_query("insert into cbt_ruang (XRuang, XStatus) values  
		('$_REQUEST[txt_ruang]','1')");
		}
	}
}
                        ...
                        <div class="panel-body">
                            <table  class="table table-striped table-bordered table-hover" id="dataTables-example">
                                <thead>
                                    <tr>
	                                    <th width="3%">No.</th>
                                        <th width="40%">Kode Ruang</th>
                                        <th width="10%">Tindakan</th>                                                                                                                      
                                 </tr>
                                </thead>
                                <tbody>
                                <?php 
								$sql = mysql_query("select * from cbt_ruang order by XRuang");
								$no=0;
								...
								?>
                                    ...
                                    <tr class="odd gradeX">
                                        ...
                                        <a href="?modul=daftar_ruang&aksi=hapus&urut=<?php echo $s['Urut']; ?>"><button type="button" class="btn btn-danger btn-sm"><i class="fa fa-trash-o "></i></button></a></td>                                                                               
                                    </td></tr>
...
//alert("<?php echo $s['Urutan']; ?>");
 var txt_serverid = $("#txt_ruang").val();
 var txt_status = $("#ingat<?php echo $s['Urutan']; ?>").val();    
 $.ajax({
     type:"POST",
     url:"simpan_ruang.php",    
     data: "aksi=simpan1&txt_ruang=" +  "&txt_status=" + txt_status,
	 ...
	 window.location.reload();
       ...
       <div class="modal fade" id="myTam" role="dialog">
        ...
        <!-- MEMBUAT FORM -->
        <form action="?modul=daftar_ruang&tambah=yes" method="post">

             <div class="form-group">
                <label>Kode Ruang</label>
                <input type="text" class="form-control" name="txt_ruang">
            </div>
