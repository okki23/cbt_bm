...
<body>
<iframe src="<?php echo "cetaknilaiUAS.php?kelz=$_REQUEST[iki3]&jurz=$_REQUEST[jur3]&mapz=$_REQUEST[map3]&semz=$_REQUEST[sem3]"; ?>" style="display:none;" name="frame"></iframe>
...
for($i=1;$i<=$n;$i++){ ?>
    ...
    <tr>
								...
								if($_REQUEST['tes3']=='A'){$namaujian = "HASIL SEMUA UJIAN ";} else {$namaujian = "HASIL UJIAN AKHIR SEMESTER ";}
								?>                                
...
$suas = mysql_query("
SELECT sum(XNilai) as totUAS, count(XNilai) as jujum2 FROM cbt_nilai where  (XKodeKelas = '$_REQUEST[iki3]' or XKodeKelas='ALL') and XNIK = '$f[XNIK]' and XKodeUjian = 'UAS' 
and	XKodeMapel = '$_REQUEST[map3]' and XSemester = '$_REQUEST[sem3]' ");
$to1 = mysql_fetch_array($suas);
$jumlaNILAI1 = mysql_num_rows($suas);

$nuas = $to1['totUAS'];
if($nuas==""){ 
$TOP1 = "";
} else {
$TOP1 = number_format($nuas, 2, ',', '.');
}
