...
$sql = mysql_query("delete from cbt_pilihan where Urut = '$_REQUEST[urut]'");
}


if(isset($_REQUEST['simpan'])){
	$sql = mysql_query("update cbt_pilihan set XPilihan = '$_REQUEST[txt_pilih]' where Urut = '$_REQUEST[urut]'");
}

if(isset($_REQUEST['tambah'])){

$sqlcek = mysql_num_rows(mysql_query("select * from cbt_pilihan where XPilihan = '$_REQUEST[txt_pilih]'"));
	if($sqlcek>0){
	$message = "Kode Mapel Pilihan SUDAH ADA";
	echo "<script type='text/javascript'>alert('$message');</script>";
	} else {
		if(!$_REQUEST['txt_pilih']==""){
		$sql = mysql_query("insert into cbt_pilihan (XPilihan, XStatus) values  
		('$_REQUEST[txt_pilih]','1')");
		}
	}
}
...
<?php echo "</a>";?>                            
                        ...
                        <div class="panel-body">
                            <table  class="table table-striped table-bordered table-hover" id="dataTables-example">
                                ...
                                <tbody>
                                <?php 
								$sql = mysql_query("select * from cbt_pilihan order by XPilihan");
								$no=0;
								...
								?>
                                    ...
                                    <tr class="odd gradeX">
                                        ...
                                        <a href="?modul=daftar_pilihan&aksi=hapus&urut=<?php echo $s['Urut']; ?>"><button type="button" class="btn btn-danger btn-sm"><i class="fa fa-trash-o "></i></button></a></td>                                                                               
                                    </td></tr>
...
//alert("<?php echo $s['Urutan']; ?>");
 var txt_serverid = $("#txt_pilih").val();
 var txt_status = $("#ingat<?php echo $s['Urutan']; ?>").val();    
 $.ajax({
     type:"POST",
     url:"simpan_pilihan.php",    
     data: "aksi=simpan1&txt_pilih=" + txt_pilih, "&txt_status=" + txt_status,
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <h4 class="modal-title">Tambah Mapel Pilihan</h4>
                </div>
                <div class="modal-body">
        <!-- MEMBUAT FORM -->
        <form action="?modul=daftar_pilihan&tambah=yes" method="post">

             <div class="form-group">
                <label>Kode Mapel</label>
                <input type="text" class="form-control" name="txt_pilih">
            </div>
