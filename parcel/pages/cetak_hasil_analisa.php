...
<html>
<head>
<title>MY CBT - CETAK HASIL</title>
</head>
<body>
...
<iframe src="<?php echo "cetaknilaiakhir.php?kelas=$_REQUEST[iki3]&jur=$_REQUEST[jur3]"; ?>" style="display:none;" name="frame"></iframe>
...
for($i=1;$i<=$n;$i++){ ?>
    ...
    <tr>
								...
								if($_REQUEST['tes3']=='A'){$namaujian = "HASIL AKHIR";} else {$namaujian = "HASIL UJIAN $rs1";}
								?>                                
...
while($f= mysql_fetch_array($cekQuery1)){
	$uh = mysql_query("SELECT sum(XNilai) as totUH FROM cbt_nilai where XKodeKelas = '$_REQUEST[iki3]' and XNIK = '$f[XNIK]' and XKodeUjian = 'UH' and XKodeMapel = '$_REQUEST[map3]' and XSemester = '1' ");
	$tuh = mysql_fetch_array($uh);
	if(isset($tuh['totUH'])){$totUH1 = number_format($tuh['totUH'], 2, ',', '.');} else {$totUH1="";}

	$uts = mysql_query("SELECT sum(XNilai) as totUTS FROM cbt_nilai where XKodeKelas = '$_REQUEST[iki3]' and XNIK = '$f[XNIK]' and XKodeUjian = 'UTS' and XKodeMapel = '$_REQUEST[map3]' and XSemester = '1' ");
	$tuts = mysql_fetch_array($uts);
	if(isset($tuts['totUTS'])){$totUTS1 = number_format($tuts['totUTS'], 2, ',', '.');} else {$totUTS1="";}	


	$uas = mysql_query("SELECT sum(XNilai) as totUAS FROM cbt_nilai where XKodeKelas = '$_REQUEST[iki3]' and XNIK = '$f[XNIK]' and XKodeUjian = 'UAS' and XKodeMapel = '$_REQUEST[map3]' and XSemester = '1' ");
	$tuas = mysql_fetch_array($uas);
	if(isset($tuas['totUAS'])){$totUAS1 = number_format($tuas['totUAS'], 2, ',', '.');} else {$totUAS1="";}	

	$uh2 = mysql_query("SELECT sum(XNilai) as totUH2 FROM cbt_nilai where XKodeKelas = '$_REQUEST[iki3]' and XNIK = '$f[XNIK]' and XKodeUjian = 'UH' and XKodeMapel = '$_REQUEST[map3]' and XSemester = '2' ");
	$tuh2 = mysql_fetch_array($uh2);
	$totUH2 = $tuh2['totUH2'];

	$uts2 = mysql_query("SELECT sum(XNilai) as totUTS2 FROM cbt_nilai where XKodeKelas = '$_REQUEST[iki3]' and XNIK = '$f[XNIK]' and XKodeUjian = 'UTS' and XKodeMapel = '$_REQUEST[map3]' and XSemester = '2' ");
	$tuts2 = mysql_fetch_array($uts2);
	$totUTS2 = $tuts2['totUTS2'];

	$uas2 = mysql_query("SELECT sum(XNilai) as totUAS2 FROM cbt_nilai where XKodeKelas = '$_REQUEST[iki3]' and XNIK = '$f[XNIK]' and XKodeUjian = 'UAS' and XKodeMapel = '$_REQUEST[map3]' and XSemester = '2' ");
