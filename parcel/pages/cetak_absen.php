...
<body>
<iframe src="<?php echo "cetakabsen.php?server=$_REQUEST[server]&kelas=$_REQUEST[iki1]&jur=$_REQUEST[jur1]&sesi=$_REQUEST[sesi1]&ruang=$_REQUEST[ruang1]&mapel=$_REQUEST[mapel1]&tanggal=$_REQUEST[tanggal1]&mulai=$_REQUEST[mulai1]&akhir=$_REQUEST[akhir1]"; ?>" style="display:none;" name="frame"></iframe>
...
$cekQuery = mysql_query("SELECT * FROM cbt_siswa where  XKodeSekolah = '$_REQUEST[server]' and XKodeKelas = '$_REQUEST[iki1]' and  XKodeJurusan = '$_REQUEST[jur1]'  and  XSesi = '$_REQUEST[sesi1]' and  XRuang = '$_REQUEST[ruang1]'  ");
}else{
...
for($i=1;$i<=$n;$i++){ ?>
    ...
    <td>
	<b><center><font size="+1">DAFTAR HADIR PESERTA UBK </font>
    <br><font size="+1">UJIAN SEKOLAH  BERBASIS KOMPUTER</font>
	<br><font size="+1"><?php echo $namsek; ?></font>
    <br><font size="+1"><b>TAHUN PELAJARAN : <?php $sqk = mysql_query("select * from cbt_tahunakademik group by Xtahunakadmik");$row=mysql_fetch_array($sqk,MYSQL_ASSOC);
								 echo $row['Xtahunakadmik']; ?></b></font>
	</b>
	</center>
	<br>
 </td> 
  ...
  <table>
    <tr height="30">
  <td height="30" width="21%">Hari/Tanggal</td>
  <td height="30" width="2%">:</td>
  <td height="30" width="1%" ><?php echo $hari; ?> </td>
  <td height="30" width="1%">,</td>
  <td height="30" width="41%" ><?php echo $tgl; ?> <?php echo $bln; ?> <?php echo $thn; ?> </td>
  <td height="30" width="1%"></td>
  <td height="30" width="16%" style="margin-left:10px"> Pukul </td>
    <td height="30" width="1%">:</td>
...
<?php
 if(isset($_REQUEST['iki1'])){ 
$cekQuery1 = mysql_query("SELECT * FROM cbt_siswa where XKodeSekolah = '$_REQUEST[server]' and XKodeKelas = '$_REQUEST[iki1]' and  XKodeJurusan = '$_REQUEST[jur1]'  and  XSesi = '$_REQUEST[sesi1]' and  XRuang = '$_REQUEST[ruang1]' limit $batas,$jumlahn");
}else{
