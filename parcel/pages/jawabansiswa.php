...
<a href=?modul=analisajawaban&soal=<?php echo $_REQUEST['soal']; ?>>
                                        <button type="button" class="btn btn-success btn-sm" style="margin-top:5px; margin-bottom:5px"><i class="glyphicon glyphicon-th-list"></i> Kembali ke Daftar</i></button></a>
<button type="button" class="btn btn-default btn-sm" onClick="frames['frame'].print()" style="margin-top:5px; margin-bottom:5px"><i class="glyphicon glyphicon-print"></i> Cetak Hasil Ujian
</button>
...
$sqljawaban = mysql_query("SELECT count( XNilai ) AS HasilUjian FROM `cbt_jawaban` WHERE XKodeSoal = '$var_soal' and XUserJawab = '$_REQUEST[siswa]' and XNilai = '1' and XTokenUjian = '$var_token'");
	...
	$hasil_pil = $jumbenar;	
	if($var_pil==0){
	$nilai_pil = round(($jumbenar*$per_pil),2);
	}else{
