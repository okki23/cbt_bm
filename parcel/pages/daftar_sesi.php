...
$sql = mysql_query("delete from cbt_sesi where Urut = '$_REQUEST[urut]'");
}


if(isset($_REQUEST['simpan'])){
	$sql = mysql_query("update cbt_sesi set XSesi = '$_REQUEST[txt_sesi]' where Urut = '$_REQUEST[urut]'");
...
$sqlcek = mysql_num_rows(mysql_query("select * from cbt_sesi where XSesi = '$_REQUEST[txt_sesi]'"));
	if($sqlcek>0){
	$message = "Kode Sesi SUDAH ADA";
	echo "<script type='text/javascript'>alert('$message');</script>";
	} else {
		if(!$_REQUEST['txt_sesi']==""){
		$sql = mysql_query("insert into cbt_sesi (XSesi, XStatus) values  
		('$_REQUEST[txt_sesi]','1')");
		}
	}
}
                        ...
                        <div class="panel-body">
                            <table  class="table table-striped table-bordered table-hover" id="dataTables-example">
                                <thead>
                                    <tr>
	                                    <th width="2%">No.</th>
                                        <th width="40%">Kode Sesi</th>
                                        <th width="6%">Tindakan</th>                                                                                                                      
                                 </tr>
                                </thead>
                                <tbody>
                                <?php 
								$sql = mysql_query("select * from cbt_sesi order by XSesi");
								$no=1;
								...
								?>
                                    ...
                                    <tr class="odd gradeX">
                                        ...
                                        <a href="?modul=daftar_sesi&aksi=hapus&urut=<?php echo $s['Urut']; ?>"><button type="button" class="btn btn-danger btn-sm"><i class="fa fa-trash-o "></i></button></a></td>                                                                               
                                    </td></tr>
...
//alert("<?php echo $s['Urutan']; ?>");
 var txt_serverid = $("#txt_jur").val();
 var txt_status = $("#ingat<?php echo $s['Urutan']; ?>").val();    
 $.ajax({
     type:"POST",
     url:"simpan_sesi.php",    
     data: "aksi=simpan1&txt_jur=" +  "&txt_status=" + txt_status,
	 ...
	 window.location.reload();
       ...
       <div class="modal fade" id="myTam" role="dialog">
        ...
        <!-- MEMBUAT FORM -->
        <form action="?modul=daftar_sesi&tambah=yes" method="post">

             <div class="form-group">
                <label>Kode Sesi</label>
                <input type="text" class="form-control" name="txt_sesi">
            </div>
...
<div class="modal fade" id="myModal" role="dialog">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">Edit Data Sesi</h4>
                </div>
                <div class="modal-body">
